...
| Implement 4 domain agents | `src/agents/{customer,order,payment,delivery}_agent.py` | 4 agents hoạt động độc lập | `python src/agents/customer_agent.py` |
| Implement Policy Engine | `src/agents/policy_engine.py` | EC_POLICY_V2 rules (6 primary + 5 secondary) | `python test_single_case.py` |
| Process 50 cases | `main.py` | 50 output JSONs, metadata.json | `python main.py` |
| Validate outputs | `src/validators/schema_validator.py` | 100% schema compliance | Review `output/*.json` |

**Output cụ thể:**
- 50 output JSON files trong `output/` folder
- `logging/metadata.json` với statistics: 50/50 success, 0.66s total time
- `submission.zip` (47 KB) chứa 50 output files
