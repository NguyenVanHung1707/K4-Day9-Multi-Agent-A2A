...
| **Pipeline Orchestrator & Fast Rebuilder** | `run_pipeline.py`, `fast_rebuild.py` | 50 file `input/EC_0xx.json` | 50 file `output_v4/EC_0xx.json`, `trace_output_v4.jsonl`, `metadata_output_v4.json`, `output_v4.zip` | Hoàn thành |
...
| **Xử lý 50 case đối soát khiếu nại** | `output_v4/EC_001.json` - `output_v4/EC_050.json` | 50 JSON outputs chuẩn 100% schema | Đã kiểm tra 50/50 file hợp lệ |
| **Tạo nhật ký chạy Trace** | `trace_output_v4.jsonl` | Nhật ký phân tích theo từng phase cho 50 case | 50 dòng JSONL |
| **Đăng ký thông số kỹ thuật Model** | `metadata_output_v4.json` | Khai báo mô hình LLM `llama-3.1-8b-instant` (8B $\le$ 10B) | File JSON chuẩn cấu trúc |
| **Đóng gói file nộp bài Portal** | `output_v4.zip` | File zip nộp bài chứa đúng 50 file JSON chuẩn portal | Autograder chấm đạt **81.6516 điểm** |
...
- **Bằng chứng quyết định phù hợp:** Toàn bộ 50 case chạy với tốc độ 2.53s, tổng điểm đạt **81.6516 điểm**.
